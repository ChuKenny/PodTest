...
#import "SGPKeychain.h"

NSString *const kSGKeychainErrorDomain = @"com.samsoffes.SGKeychain";

NSString *const kSGKeychainAccountKey = @"acct";
NSString *const kSGKeychainCreatedAtKey = @"cdat";
NSString *const kSGKeychainClaSGKey = @"labl";
NSString *const kSGKeychainDescriptionKey = @"desc";
NSString *const kSGKeychainLabelKey = @"labl";
NSString *const kSGKeychainLastModifiedKey = @"mdat";
NSString *const kSGKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef SGKeychainAccessibilityType = NULL;
#endif

@interface SGPKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation SGPKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = SGKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kSGKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (SGKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return SGKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (SGKeychainAccessibilityType) {
		CFRelease(SGKeychainAccessibilityType);
	}
	SGKeychainAccessibilityType = accessibilityType;
}
#endif
